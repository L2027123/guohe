import{a as $,j as e,b as be,c as je}from"./index-DgdXw88Y.js";import{b as Ne,u as fe,r as v}from"./react-vendor-DSYpD63Q.js";import{h as ve,W as R,j as Z,U as T,A as M,N as ee,L as se,ao as te,k as ye,af as ke,q as we,z as $e,ak as Ae,ap as ze}from"./lucide-NCvXOZx7.js";import"./zustand-DOacgZDb.js";function Fe(){var W,K,U,G,q,X,Q,Y;const S=Ne(),b=fe(),y=$(s=>s.currentProjectId),re=$(s=>s.projects),ae=$(s=>s.styleDNA),ie=$(s=>s.ensureAccountMemory),le=$(s=>s.updateMemoryPatterns),ne=$(s=>s.performanceRecords),A=re.find(s=>s.id===y),F=ae[y],[j,E]=v.useState("video"),[r,ce]=v.useState(null);v.useEffect(()=>{var s;((s=b.state)==null?void 0:s.source)==="video-director-script"&&(E("script"),ce({hook:b.state.hook||null,script:b.state.script||"",structure:b.state.structure||null,title:b.state.title||"",cover:b.state.cover||null,shootingPlan:b.state.shootingPlan||null,projectId:b.state.projectId||null}))},[b.state]);const[d,O]=v.useState(null),[P,V]=v.useState(!1),[n,_]=v.useState(null),[D,f]=v.useState(""),oe=()=>{const s=localStorage.getItem("contentos_api_key");return s||(f("请先在设置页面配置 DeepSeek API Key"),null)},H=s=>s<1024?`${s}B`:s<1024*1024?`${(s/1024).toFixed(1)}KB`:`${(s/1024/1024).toFixed(1)}MB`,de=s=>{var a;const t=(a=s.target.files)==null?void 0:a[0];if(!t)return;if(!t.name.match(/\.(mp4|mov)$/i)){f("只支持 mp4 / mov 格式");return}f("");const x=URL.createObjectURL(t),i=document.createElement("video");i.preload="metadata",i.src=x;const o=(c={})=>{O({videoUrl:x,fileName:t.name,fileSize:t.size||0,fileType:t.type||"video/unknown",duration:0,hasVideo:!1,hasAudio:!1,width:0,height:0,...c})};i.onloadedmetadata=()=>{var l;const c=i.videoWidth?{hasVideo:!0,width:i.videoWidth,height:i.videoHeight}:{};o({duration:Math.round(i.duration||0),hasAudio:(((l=i.audioTracks)==null?void 0:l.length)||0)>0||i.mozHasAudio||!!i.webkitAudioDecodedByteCount,...c})},i.onerror=()=>o()},xe=()=>{var i,o,a,c,l,m;const s=A?`账号名称：${A.name}
平台：${A.platform||"小红书"}
领域：${A.category||"未指定"}
目标受众：${A.targetAudience||"未指定"}`:"账号信息未配置",t=F?`内容风格：${F.toneDescription||"未分析"}
常用表达：${((i=F.signaturePhrases)==null?void 0:i.join("、"))||"无"}`:"风格未分析";if(j==="script"&&r){const h=(o=r.hook)!=null&&o.text?`${r.hook.text}（类型：${r.hook.type||"未指定"}，原理：${r.hook.whyItWorks||"无"}）`:"未提供",N=(()=>{var w;const u=(w=r.structure)==null?void 0:w.contentFlow;return!u||u.length===0?"未提供结构":u.map(z=>`[${z.order}] ${z.segment} | 用户问题：${z.viewerQuestion||"-"} | 情绪：${z.emotion||"-"} | 脚本：${(z.script||"").slice(0,100)}`).join(`
